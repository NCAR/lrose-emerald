function pstruct = paramparses(pstruct,parsearray,varlist,warnoff,defstr)
% usage  paramparses(pstruct,parsearray,varlist,quiet)
%  pstruct is a struct containing the current values.
...
%              before paramparses is run.  If the matlab warnings are 
...
%  the paramparses parses or not, that variable depending
...
%   Thus, after the paramparses statement in myfun,
...
if nargin < 2 | isempty(parsearray)
  return;
end;
...
if nargin==4 
  switch warnoff
   ...
   case {4,'none_skip'}
    ...
    errstate = 'none';
   otherwise
    error('invalid option for paramparses');
  end;
end;
...
for l = 1:length(fields)
  if ischar(fields{l})
	...
	ind = length(fields{l})+1;
      ...
      varthere = isfield(pstruct,fields{l}(1:ind(1)-1));
    ...
    %  LAST_PARAMPARSE_DEFAULTS = pstruct;
...
% 	       'this variable before this paramparses call (set it [])']);
%       disp('*****************************************');
%     end;
    ...
    %  of the way matlab allocates variables that
...
if ~isempty(PARAMPARSE_LOG)
  if ~isempty(pstruct)
    if ~isstruct(PARAMPARSE_LOG)
      ...
      PARAMPARSE_LOG(end).data = pstruct;
    end;
  end;
else
  clear global PARAMPARSE_LOG;
